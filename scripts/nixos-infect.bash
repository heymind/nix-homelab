...
infect() {
  ...
  NIX_INSTALL_URL="${NIX_INSTALL_URL:-https://mirrors.tuna.tsinghua.edu.cn/nix/latest/install}"
  ...
  nix-channel --add "https://mirrors.tuna.tsinghua.edu.cn/nix-channels/$NIX_CHANNEL" nixos
  nix-channel --update

  nix_config=~/.config/nix
  mkdir -p $nix_config
  echo "substituters = https://mirrors.tuna.tsinghua.edu.cn/nix-channels/store https://cache.nixos.org/" > $nix_config/nix.conf
